...
#min(amata18$g_m_sq[amata18$g_m_sq > 0]) # 0.000539607
#min(amata19$g_m_sq[amata19$g_m_sq > 0]) # 0.004464286

pop_biom18_2 <- lm(log(g_m_sq + 0.00025) ~ as.factor(Block) + Crop_ID + Corn_weed_management + Crop_ID:Corn_weed_management, data = amata18)
#anova(pop_biom18_2)
...
## How did crop identity and corn weed management affect AMATA aboveground mass in 2019? ##
#Find the smallest non-zero value in aboveground mass: 
#min(amata19$g_m_sq[amata19$g_m_sq > 0])

pop_biom19_2 <- lm(log(g_m_sq + 0.002) ~ Block +
...
#min(amata18$plant_m_sq[amata18$plant_m_sq > 0]) #0.05396072
#min(amata19$plant_m_sq[amata19$plant_m_sq > 0]) #0.05396072

pop_dens18_3 <- lm(log(plant_m_sq + 0.025) ~ as.factor(Block) + Crop_ID + Corn_weed_management + Crop_ID:Corn_weed_management, data = amata18)
...
pop_dens19_3 <- lm(log(plant_m_sq + 0.025) ~ Block +
                     Crop_ID + Corn_weed_management +
                     Crop_ID:Corn_weed_management, 
                   data = amata19)
...
#Round F-ratio to 1 decimal place, if needed: dens_jt_19$F.ratio <- round(dens_jt_19$F.ratio, 1)
...
#biom_dens_round <- sapply(biom_dens[,c(3,5)], as.numeric)
```

```{r pop-biom-dens-jt, echo=FALSE}
biom_dens[,-c(8:10)] %>%
kableExtra::kbl(longtable = FALSE, booktabs = TRUE, digits = 2,
  ...
  kableExtra::add_header_above(c(c(" " = 3, 
                     "Population \n aboveground mass" = 2, 
                     "Population \n stand density" = 2,
                     "Population \n aboveground mass" = 2, 
                     "Population \n stand density" = 2))) %>%
    kableExtra::add_header_above(c(" " = 3, "2018" = 4, "2019" = 4)) %>% 
...
biom_emm_18 <- emmeans(pop_biom18_2, "Crop_ID", type="response") 
...
```{r, include=FALSE}
bp18 <- plot(biom_emm_18, comparisons = TRUE) + 
    theme_bw() + 
...
bp19 <- plot(biom_emm_19, comparisons = TRUE) + 
    theme_bw() + 
    theme(text=element_text(size=10))+
    coord_flip() +
  xlab(expression(g/m^2)) +
...
dp18 <- plot(dens_emm_18, comparisons = TRUE) + 
    theme_bw() + 
    theme(text=element_text(size=10))+
    coord_flip() +
...
dp19 <- plot(dens_emm_19, comparisons = TRUE) + 
    theme_bw() + 
   theme(text=element_text(size=10))+
    coord_flip() +
  xlab(expression(plant/m^2)) +
...
gridExtra::grid.arrange(bp18, bp19, dp18, dp19, nrow = 2) # use gridExtra package 
```  
...
pop_biom_corn_18 <- emmeans(pop_biom18_2, ~ Crop_ID , infer = c(FALSE,TRUE),  type = "response", at = list(Crop_ID = c( "C2", "C3", "C4")))
pop_biom_corn_ct18 <- contrast(pop_biom_corn_18, "pairwise", type = "response", infer = c(FALSE,TRUE)) %>% print(export = TRUE)

pop_biom_soy_18 <- emmeans(pop_biom18_2, ~ Crop_ID , infer = c(FALSE,TRUE),  type = "response", at = list(Crop_ID = c( "S2", "S3", "S4")))
pop_biom_soy_ct18 <- contrast(pop_biom_soy_18, "pairwise", type = "response", infer = c(FALSE,TRUE)) %>% print(export = TRUE) #since S3 not significantly diff from S4, can do S2 vs (s3+S4)/2

pop_biom_oat_18 <- emmeans(pop_biom18_2, ~ Crop_ID , infer = c(FALSE,TRUE),  type = "response", at = list(Crop_ID = c( "O3", "O4")))
pop_biom_oat_ct18 <- contrast(pop_biom_oat_18, "pairwise", type = "response", infer = c(FALSE,TRUE)) %>% print(export = TRUE)
...
pop_biom_corn_19 <- emmeans(pop_biom19_2, ~ Crop_ID , infer = c(FALSE,TRUE),  type = "response", at = list(Crop_ID = c( "C2", "C3", "C4")))
pop_biom_corn_ct19 <- contrast(pop_biom_corn_19, "pairwise", type = "response", infer = c(FALSE,TRUE)) %>% print(export = TRUE)

pop_biom_soy_19 <- emmeans(pop_biom19_2, ~ Crop_ID , infer = c(FALSE,TRUE),  type = "response", at = list(Crop_ID = c( "S2", "S3", "S4")))
pop_biom_soy_ct19 <- contrast(pop_biom_soy_19, "pairwise", type = "response", infer = c(FALSE,TRUE)) %>% print(export = TRUE) #since S3 not significantly diff from S4, can do S2 vs (s3+S4)/2

pop_biom_oat_19 <- emmeans(pop_biom19_2, ~ Crop_ID , infer = c(FALSE,TRUE),  type = "response", at = list(Crop_ID = c( "O3", "O4")))
pop_biom_oat_ct19 <- contrast(pop_biom_oat_19, "pairwise", type = "response", infer = c(FALSE,TRUE)) %>% print(export = TRUE)
...
# crop contrast, averaged over rotations
pop_biom_crop19 <- emmeans(biom_emm_19_grid_crop, ~ Crop, infer = c(FALSE,TRUE),  type = "response") 

pop_biom_crop_ct19 <- contrast(pop_biom_crop19, "pairwise", type = "response", infer = c(FALSE,TRUE)) %>% print(export = TRUE)
...
pop_dens_corn18 <-  emmeans(pop_dens18_3, ~ Crop_ID , infer = c(FALSE,TRUE),  type = "response", at = list(Crop_ID = c( "C2", "C3", "C4")))

pop_dens_corn_ct18 <- contrast(pop_dens_corn18 , "pairwise", type = "response", infer = c(FALSE,TRUE)) %>% print(export = TRUE)

pop_dens_soy18 <- emmeans(pop_dens18_3, ~ Crop_ID, infer = c(FALSE,TRUE),  type = "response", at = list(Crop_ID = c("S2", "S3", "S4"))) 

pop_dens_soy_ct18 <- contrast(pop_dens_soy18, "pairwise", type = "response", infer = c(FALSE,TRUE)) %>% print(export = TRUE)

pop_dens_oat18 <- emmeans(pop_dens18_3, ~ Crop_ID, infer = c(FALSE,TRUE),  type = "response", at = list(Crop_ID = c("O3", "O4")))

pop_dens_oat_ct18 <- contrast(pop_dens_oat18, "pairwise", type = "response", infer = c(FALSE,TRUE))  %>% print(export = TRUE)
...
pop_dens_crop18 <- emmeans(dens_emm_18_grid_crop, ~ Crop, infer = c(FALSE,TRUE),  type = "response") 

pop_dens_crop_ct18 <- contrast(pop_dens_crop18, "pairwise", type = "response", infer = c(FALSE,TRUE)) %>% print(export = TRUE)
...
pop_dens_corn19 <-  emmeans(pop_dens19_3, ~ Crop_ID , infer = c(FALSE,TRUE),  type = "response", at = list(Crop_ID = c( "C2", "C3", "C4")))

pop_dens_corn_ct19 <- contrast(pop_dens_corn19, "pairwise", type = "response", infer = c(FALSE,TRUE)) %>% print(export = TRUE)

pop_dens_soy19 <- emmeans(pop_dens19_3, ~ Crop_ID, infer = c(FALSE,TRUE),  type = "response", at = list(Crop_ID = c("S2", "S3", "S4"))) 

pop_dens_soy_ct19 <- contrast(pop_dens_soy19, "pairwise", type = "response", infer = c(FALSE,TRUE)) %>% print(export = TRUE)

pop_dens_oat19 <- emmeans(pop_dens19_3, ~ Crop_ID, infer = c(FALSE,TRUE),  type = "response", at = list(Crop_ID = c("O3", "O4")))

pop_dens_oat_ct19 <- contrast(pop_dens_oat19, "pairwise", type = "response", infer = c(FALSE,TRUE))  %>% print(export = TRUE)
...
pop_dens_crop19 <- emmeans(dens_emm_19_grid_crop, ~ Crop, infer = c(FALSE,TRUE),  type = "response")

pop_dens_crop_ct19 <- contrast(pop_dens_crop19, "pairwise", type = "response", infer = c(FALSE,TRUE)) %>% print(export = TRUE)
...
sexr18_dens_emm_1 <- emmeans(sexr18_dens_glm3, c("Crop_ID" , "Corn_weed_management" ), type = "response", infer = c(TRUE,TRUE), at=list(plant_m_sq=1))


dens_1 <- plot(sexr18_dens_emm_1, comparisons = TRUE, by = "Corn_weed_management") + 
    theme_bw() + 
    theme(text=element_text(size=10),
...
sexr18_dens_emm_5 <- emmeans(sexr18_dens_glm3, c("Crop_ID" , "Corn_weed_management" ), type = "response", infer = c(TRUE,TRUE), at=list(plant_m_sq=5))

dens_5 <- plot(sexr18_dens_emm_5, comparisons = TRUE, by = "Corn_weed_management") + 
    theme_bw() + 
    theme(text=element_text(size=10),
          strip.text.x = element_text(size = 5)) + 
    coord_flip() +
...
sexr18_dens_emm_50 <- emmeans(sexr18_dens_glm3, c("Crop_ID" , "Corn_weed_management" ), type = "response", infer = c(TRUE,TRUE),at=list(plant_m_sq=50))

dens_50 <- plot(sexr18_dens_emm_50, comparisons = TRUE, by = "Corn_weed_management") + 
    theme_bw() + 
    theme(text=element_text(size=10),
          strip.text.x = element_text(size = 5)) +
    coord_flip() +
...
sexr18_dens_emm_500 <- emmeans(sexr18_dens_glm3, c("Crop_ID" , "Corn_weed_management" ), type = "response", infer = c(TRUE,TRUE),at=list(plant_m_sq=500))

dens_500 <- plot(sexr18_dens_emm_500, comparisons = TRUE, by = "Corn_weed_management") + 
    theme_bw() + 
    theme(text=element_text(size=10),
          strip.text.x = element_text(size = 5)) +
  geom_vline(xintercept=0.5, size=0.5, linetype="dashed") + 
...
```{r, echo=FALSE}
## 2019 sex ratio as is, meaning before imputation

sexed19_mis <- read_csv(here("2_Data/Clean/sexed19_mis.csv"), 
    col_types = cols(Plot = col_factor(), 
        Quadrat = col_factor(), 
        Cohort = col_factor(), 
        Block = col_factor(), 
...
sexed19_imp1_wide <- parlmice(sexed19_resp1 , cluster.seed = 500, print = FALSE, n.core = 4, n.imp.core = 6) #cluster.seed = 500, identical to Appendix B
...
sexr19_imp_p <- ggplot(sexed19_imp1_jt_df, aes(x = imp, y = p.value)) + 
  geom_point(aes(shape=factor(Sig.code),
                 ...
                 size = .5, 
                 alpha = .5)) +   #shape = 1) +
  facet_wrap(~ `model term`, ncol = 2, labeller = as_labeller(sexr_19_panel_names))+
  ggtitle("(B) p.values") + 
...
```{r, include=FALSE}
## Imputed 2019 sex ratio with m = 24 iterations, July 16, 2021.

sexed19_m24 <- read_csv("../2_Data/Clean/sexed19_m24.csv",
...
##merge imputed with population density and mass ?? remove??
...
sexed19_m24_Jan29 <- read_csv("../2_Data/Clean/sexed19_m24_Jan29_22.csv",
                   col_types = cols(Male = col_integer(), 
                                    Female = col_integer()))
...
# A customized function from emmeans()
...
## Apply the "mod_emm_h" model on 24 sets of imputed data 
sexed19_imp1_emm_h <- sexed19_m24_Jan29  %>% split(.$.imp) %>% map(mod_emm_h) 
