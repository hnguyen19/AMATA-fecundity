...
library(kableExtra)
source(here("8_Analysis_scripts/functions.R"))
...
sexed19_imp1_full <- sexed19_imp1_jt_df %>% 
  filter(is.na(note)) # sets 1, 4, 14, 21, 22, and 24, among the 24 imputed sets are full - that allow estimable effects of crop ID, corn weed management, and the interaction of these two factors

sexed19_imp1_notfull <-  sexed19_imp1_jt_df %>% 
  filter(!is.na(note)) # the other 18 sets that are not fully imputed, resulting nonestimable effects of corn weed management.
...
sexed19_imp3_not_full <- sexed19_imp3_jt_df %>% 
  filter(!is.na(note)) # nrow = 6312, n sets = 6312/3 = 2104

n3 <- nrow(sexed19_imp3_full)/4 #296; 296/4 = 74 full + 6312/3 = 2104 not full = 2400
...
kbl(all_imp_n_ordered_wide, 
  ...
  landscape() %>%
  footnote(general = "Some zeros are due to rounding. C2: corn in the 2-year rotation, C3: corn in the 3-year rotation, C4: corn in the 4-year rotation, S2: soybean in the 2-year rotation, S3: soybean in the 3-year rotation; S4: soybean in the 4-year rotation, O3: oat in the 3-year rotation, and A4: alfalfa in the 4-year rotation", threeparttable = TRUE, footnote_as_chunk = TRUE)
...
amata18 <- read_csv("../2_Data/Clean/AMATA_18_clean.csv", 
...
#error from here
## Read in 2019 raw data
amata19_raw <- read_csv("../2_Data/Raw/AMATA_19_raw.csv",
                    col_types = cols(Plot = col_factor(), 
                                     Block = col_factor(), 
                                     Crop = col_factor(),
                                     Crop_ID = col_factor(),
...
kbl(imp_orginal, booktabs = TRUE, 
  ...
  column_spec(c(2,5,15,22,23,25), bold = TRUE) %>% # sets 1, 4, 14, 21, 22, and 24, 
  ...
  kable_styling(latex_options =  c("scale_down")) %>%
  landscape() %>%
  footnote(general = "Bold columns are imputations that produced full sets of data (six among 24 imputed sets) that allow estimation of the effects of crop idenity, corn weed management, and their interaction.")
```
